...
class SpotifyList extends Component {
  ...
  componentDidMount() {
    const url =
      "https://api.spotify.com/v1/me/tracks?offset=0&limit=40&market=TW";
    ...
    return (
      <div>
        <Dimmer
          ...
          put_track_msg={spData.put_track_msg}
          name={"spotify"}
        />
        {spData.msg == "success" ? (
              ...
              {spData.data.items.map((data) => {
                return (
                  <div className="spotify_box" key={data.track.id}>
                    <Grid.Row>
                      <div
                        className="sp_track_box"
                        onClick={() => this.handle_play_button(data.track)}
                      >
                        <Image
                          className="sp_play_Icon"
                          src={
                            this.state.id == data.track.id
                              ? play_Icon
                              : music_icon
                          }
                        />
                        ...
                        <div className="sp_title_box">
                          <h3
                            className={
                              this.state.id == data.track.id ? "green" : null
                            }
                          >
                            {data.track.name}
                          </h3>
                          ...
                          >
                            {this.parseDurationMs(data.track.duration_ms)}
                          </p>
                        </div>
                      </div>
                    </Grid.Row>
